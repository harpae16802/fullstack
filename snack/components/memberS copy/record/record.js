        ...
        <div className={classNames("itemgroup item1", styles["mb-0"])}>
          <div className="card">
            {Array(4).fill(1).map((_, index) => (
              <div className={classNames("card-body", styles.flexBetween)} key={index}>
                ...
                <div className={classNames("countGroup", styles.flexBetween)}>
                  <div className='d-flex align-items-center justify-items-center'>
                    <button type="button" style={{ height: "50px" }} className={classNames("btn recordBtn", styles["btn-parmary"])}>查看詳細</button>
                  </div>
                </div>
              </div>
            ))}
